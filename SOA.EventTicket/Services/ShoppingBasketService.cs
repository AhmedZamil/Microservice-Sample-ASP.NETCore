...
namespace SOA.EventTicket.Services
{
    public class ShoppingBasketService : IShoppingBasketService
    {
        ...
        public async Task RemoveLine(Guid basketId, Guid basketLineId)
        {
            await client.DeleteAsync($"/api/baskets/{basketId}/basketLines/{basketLineId}");
        }

        public async Task UpdateLine(Guid basketId, BasketLineForUpdate basketLineForUpdate)
        {
            await client.PutAsJson($"/api/baskets/{basketId}/basketLines/{basketLineForUpdate.BasketLineId}", basketLineForUpdate);
        }
    }
}
